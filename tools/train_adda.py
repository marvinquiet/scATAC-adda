...
@click.option('--adversary', 'adversary_layers', default=[64, 16],
              multiple=True)
...
def main(source, target, model, output,
    ...
    # base network
    source_ft, _ = model_fn(source_im_batch, scope='source', n_clusters=source_dataset.n_clusters)
    target_ft, _ = model_fn(target_im_batch, scope='target', n_clusters=target_dataset.n_clusters)
