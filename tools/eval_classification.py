...
def main(dataset, split, model, weights, gpu):
    ...
    net, layers = model_fn(im_batch, is_training=False, n_clusters=dataset.n_clusters)
    net = tf.argmax(net, -1)
    ...
    class_correct = np.zeros(dataset.n_clusters, dtype=np.int32)
    class_counts = np.zeros(dataset.n_clusters, dtype=np.int32)
    predicted_values, ground_truth = [], []
    for i in tqdm(range(len(split))):
        predictions, gt = sess.run([net, label_batch])
        predicted_values.append(predictions[0])
    ...
    logging.info('ARI:')
    logging.info('    ' + str(metrics.cluster.adjusted_rand_score(predicted_values, ground_truth)))
    logging.info('macroF1:')
    logging.info('    ' + str(metrics.f1_score(predicted_values, ground_truth, average='macro')))
